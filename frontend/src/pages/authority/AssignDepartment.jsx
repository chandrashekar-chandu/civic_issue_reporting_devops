...
const AssignDepartment = () => {
  ...
  // ==========================================
  // FETCH ISSUE + DEPARTMENTS
  // ==========================================
  const fetchData = async () => {
    try {
      const [issueResponse, departmentsResponse] =
        await Promise.all([
          api.get(`/issues/${id}`),
          api.get("/departments"),
        ]);
      ...
      // Pre-select already assigned department
      if (issueData.assignedDepartment) {
        const assignedId =
          issueData.assignedDepartment._id ||
          issueData.assignedDepartment.id;

        if (assignedId) {
          setSelectedDepartment(assignedId);
        }
      }
    } catch (error) {
      console.error(
        "Fetch Assignment Data Error:",
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-slate-300 text-lg">
            Route this issue to the appropriate
            department.
          </p>
        </div>
        ...
        <div className="bg-white/10 backdrop-blur-xl border border-white/10 rounded-3xl shadow-2xl p-8">
          ...
          >
            ...
            <button
              type="submit"
              disabled={
                saving || !selectedDepartment
              }
              className="w-full py-4 rounded-xl bg-gradient-to-r from-violet-500 to-purple-600 hover:from-violet-400 hover:to-purple-500 text-white font-semibold text-lg shadow-lg transition duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {saving
